...
const Requests: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("Solicitação enviada para o RH. Acompanhe em 'Em análise'.");
    } catch (e) {
      ...
      <div className="flex flex-col h-full bg-white dark:bg-slate-900 animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar pb-32">
          <div className="bg-slate-50 dark:bg-slate-800 p-5 rounded-[28px] border dark:border-slate-700">
             <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Data da Ocorrência</p>
          ...
          <div className="flex gap-4">
            <button onClick={() => setType('inclusão')} className={`flex-1 p-5 rounded-[30px] border-2 transition-all ${type === 'inclusão' ? 'border-orange-500 bg-orange-500 text-white' : 'border-slate-100 bg-slate-50 text-slate-400'}`}>
              <span className="text-xl block mb-2">📝</span>
              <span className="text-[9px] font-black uppercase">Ajuste Ponto</span>
            </button>
            <button onClick={() => setType('abono')} className={`flex-1 p-5 rounded-[30px] border-2 transition-all ${type === 'abono' ? 'border-orange-500 bg-orange-500 text-white' : 'border-slate-100 bg-slate-50 text-slate-400'}`}>
              <span className="text-xl block mb-2">🏥</span>
              <span className="text-[9px] font-black uppercase">Atestado</span>
            </button>
          </div>
          ...
          {type === 'inclusão' && (
            <div className="grid grid-cols-2 gap-3">
              {times.map((t, idx) => (
                <input key={idx} type="time" value={t} onChange={e => { const n = [...times]; n[idx] = e.target.value; setTimes(n); }} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-xs font-black border dark:border-slate-700 outline-none" />
              ))}
            </div>
          )}
          ...
          <button onClick={() => fileInputRef.current?.click()} className={`w-full p-6 rounded-3xl border-2 border-dashed ${attachmentName ? 'border-emerald-500 text-emerald-600 bg-emerald-50' : 'border-slate-200 text-slate-400'}`}>
            <span className="font-black text-[10px] uppercase">{attachmentName || 'Anexar Comprovante (Max 600KB)'}</span>
          </button>

          <div className="flex gap-4 pt-4">
            <button onClick={() => setShowCreateMode(false)} className="flex-1 py-5 bg-slate-100 rounded-[28px] text-[10px] font-black uppercase">Cancelar</button>
            <button onClick={handleSubmit} disabled={loading} className="flex-[2] py-5 bg-orange-600 text-white rounded-[28px] font-black uppercase shadow-xl disabled:opacity-50">
              {loading ? 'Sincronizando...' : 'Confirmar Envio'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900">
          ...
          <div key={req.id} className="bg-white dark:bg-slate-800 p-6 rounded-[35px] border border-slate-100 dark:border-slate-700 shadow-sm animate-in fade-in">
            <div className="flex items-center justify-between mb-4">
               <div className="flex items-center gap-3">
                 ...
                 <div>
                    <p className="text-[10px] font-black uppercase leading-none">{req.type === 'atestado' ? 'Atestado' : 'Ajuste de Ponto'}</p>
      ...
      <button onClick={() => setShowCreateMode(true)} className="fixed bottom-28 right-6 w-16 h-16 bg-orange-600 text-white rounded-full shadow-2xl flex items-center justify-center active:scale-90 border-4 border-white">
