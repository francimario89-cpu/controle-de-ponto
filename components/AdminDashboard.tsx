...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ latestRecords, company, employees, onAddEmployee, onDeleteEmployee, onUpdateEmployee, initialTab, onNavigate }) => {
  ...
  const handleApproveRequest = async (req: AttendanceRequest) => {
    try {
      ...
      alert("Aprovado e Ponto Lançado com Sucesso!");
    } catch (e) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full bg-slate-50 p-6">
        <div className="w-full max-w-sm bg-white p-10 rounded-[44px] shadow-2xl border text-center space-y-6">
           ...
           <h2 className="text-sm font-black text-slate-900 uppercase">Segurança RH</h2>
           <input 
             type="password" 
    ...
    return (
      <div className="space-y-6 animate-in fade-in">
        <div className="bg-white p-8 rounded-[40px] border shadow-sm space-y-6">
           <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Espelho de Ponto / Auditoria</h3>
        ...
        <div className="bg-white rounded-[40px] border overflow-hidden shadow-sm">
           <table className="w-full text-left">
              <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-500">
                 <tr><th className="p-5">Nome</th><th className="p-5">Data</th><th className="p-5 text-center">Horário</th><th className="p-5">Status</th></tr>
              </thead>
                    ...
                    <tr key={r.id} className={`border-b hover:bg-slate-50 transition-colors ${r.isAdjustment ? 'bg-orange-50/30' : ''}`}>
                       ...
                       <td className={`p-5 text-center font-black ${r.isAdjustment ? 'text-orange-600' : 'text-slate-800'}`}>
                         <div className="flex flex-col items-center">
                            ...
                            {r.isAdjustment && <span className="text-[7px] bg-orange-500 text-white px-2 rounded-full mt-1 uppercase tracking-tighter">AJUSTADO RH</span>}
                         </div>
                       </td>
                       <td className="p-5">
                         <span className={`text-[8px] font-black uppercase px-3 py-1.5 rounded-xl ${r.isAdjustment ? 'text-orange-600 border border-orange-200' : 'text-slate-400 border border-slate-100'}`}>
                           {r.isAdjustment ? 'Ajuste Manual' : 'Facial / GPS'}
                         </span>
                       </td>
                    </tr>
                 ))}
  ...
  if (activeTab === 'aprovacoes') {
    ...
    return (
      <div className="space-y-6 animate-in fade-in">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredRequests.map(req => (
              <div key={req.id} className="bg-white p-6 rounded-[44px] shadow-2xl border border-slate-100 flex flex-col space-y-6 animate-in zoom-in-95 duration-300">
                 <div className="flex items-center gap-4 border-b pb-4 border-slate-50">
                    ...
                    <div>
                       <h4 className="text-sm font-black uppercase text-slate-900 leading-tight">{req.userName}</h4>
                       <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">MATRÍCULA: {req.matricula}</p>
                    </div>
                 </div>
                 ...
                 <div className="bg-slate-50/50 p-6 rounded-[35px] space-y-4">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                       ...
                       <span className="text-orange-600">{req.type === 'atestado' ? 'ATESTADO MÉDICO' : 'AJUSTE DE PONTO'}</span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                    ...
                    {req.attachment && (
                      <button 
                        ...
                        className="w-full py-3 text-blue-600 font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:bg-blue-50 rounded-2xl transition-all"
                      >
                 ...
                 {req.status === 'pending' ? (
                   <div className="flex gap-3 pt-2">
                      <button onClick={() => handleRejectRequest(req.id)} className="flex-1 py-5 bg-red-50 text-red-600 rounded-3xl text-[10px] font-black uppercase active:scale-95 transition-all">Recusar</button>
                      <button onClick={() => handleApproveRequest(req)} className="flex-[2] py-5 bg-emerald-600 text-white rounded-3xl text-[10px] font-black uppercase shadow-xl shadow-emerald-100 active:scale-95 transition-all">Aprovar e Lançar</button>
                   </div>
                 ) : (
                   <div className={`py-5 rounded-3xl text-[10px] font-black uppercase text-center border ${req.status === 'approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
            ...
            {filteredRequests.length === 0 && (
              <div className="col-span-full py-32 text-center opacity-20 flex flex-col items-center">
                 ...
                 <p className="text-sm font-black uppercase tracking-[0.3em]">Nenhum registro para esta categoria.</p>
              </div>
            )}
         </div>
         ...
         {previewFile && (
           <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-6 lg:p-12 animate-in fade-in duration-300">
              <div className="bg-white w-full max-w-5xl h-full rounded-[44px] shadow-2xl overflow-hidden flex flex-col">
                 <header className="p-8 border-b flex justify-between items-center bg-slate-50">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <h4 className="font-black text-slate-900 uppercase text-sm tracking-tight">{previewFile.attachmentName}</h4>
                    ...
                    <div className="flex gap-3">
                       <button 
                        onClick={() => {
                          ...
                          link.download = previewFile.attachmentName!;
                          link.click();
                        }}
                        className="bg-orange-600 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all flex items-center gap-2"
                       >
                         📥 Baixar e Imprimir
                       </button>
                 ...
                 <div className="flex-1 bg-slate-100 overflow-y-auto p-12 flex justify-center items-center">
                    {previewFile.attachment?.startsWith('data:image') ? (
                      <img src={previewFile.attachment} className="max-w-full max-h-full object-contain rounded-2xl shadow-xl" alt="Anexo" />
                    ) : (
                      <div className="text-center space-y-4">
                         <div className="text-8xl">📄</div>
                         <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Este é um arquivo PDF ou Documento.</p>
                         <p className="text-[10px] font-bold text-slate-300">Clique em 'Baixar e Imprimir' para visualizar.</p>
                      </div>
                    )}
                 </div>
              </div>
    ...
    <div className="space-y-8 animate-in fade-in">
       <div className="bg-slate-900 p-8 rounded-[44px] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 border border-slate-800">
          <div className="space-y-1 text-center md:text-left">
             <p className="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em]">Ambiente Administrativo RH</p>
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-8 rounded-[40px] border shadow-sm relative">
             <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest mb-1">Justificativas Pendentes</p>
