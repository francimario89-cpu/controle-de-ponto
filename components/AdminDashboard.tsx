...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ latestRecords, company, employees, onAddEmployee, onDeleteEmployee, initialTab, onNavigate }) => {
      ...
      <div className="flex flex-col items-center justify-center h-full bg-slate-50 dark:bg-slate-950 p-6">
        <div className="w-full max-w-sm bg-white dark:bg-slate-900 p-10 rounded-[44px] shadow-2xl border dark:border-slate-800 text-center space-y-6 animate-in zoom-in duration-300">
           ...
           <div className="space-y-1">
             <h2 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-widest">Painel Administrativo</h2>
           ...
           <input 
             ...
             className={`w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl text-[11px] font-black text-center outline-none border-2 dark:text-white ${authError ? 'border-red-500 animate-shake' : 'border-transparent focus:border-primary/20'}`}
           />
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           ...
           <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] shadow-sm border dark:border-slate-800">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Registros Hoje</p>
              <h3 className="text-4xl font-black text-emerald-500">{stats.activeToday}</h3>
           </div>
           <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] shadow-sm border dark:border-slate-800">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Pendências</p>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[44px] p-8 border dark:border-slate-800 shadow-sm space-y-6">
           <div className="flex justify-between items-center px-2">
              ...
              <span className="text-[10px] bg-primary/5 text-primary px-3 py-1 rounded-full font-black uppercase">Módulos Ativos</span>
           </div>
           ...
           <div className="space-y-4 w-full flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                 <div className="space-y-1">
                    ...
                    <select value={reportFilter.matricula} onChange={e => setReportFilter({...reportFilter, matricula: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black outline-none border-2 border-transparent focus:border-primary/20 dark:text-white">
                      <option value="todos">FILTRAR POR EQUIPE COMPLETA</option>
                      {employees.map(e => <option key={e.id} value={e.matricula}>{e.name}</option>)}
                    </select>
                 </div>
                 <div className="space-y-1">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Mês</p>
                    <select value={reportFilter.month} onChange={e => setReportFilter({...reportFilter, month: parseInt(e.target.value)})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black dark:text-white">
                 ...
                 <div className="space-y-1">
                    ...
                    <select value={reportFilter.year} onChange={e => setReportFilter({...reportFilter, year: parseInt(e.target.value)})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black dark:text-white">
                      <option value={2024}>2024</option><option value={2025}>2025</option>
                    </select>
                 </div>
              </div>
           </div>
           <button onClick={handleDownloadReport} className="w-full md:w-auto bg-primary text-white px-10 py-5 rounded-[28px] font-black text-[11px] uppercase shadow-xl hover:scale-105 transition-all">
             Baixar Livro Mensal
           </button>
        </div>
     ...
     return (
        <div className="space-y-6 animate-in slide-in-from-right duration-500">
           <div className="flex justify-between items-center">
              ...
              <button onClick={() => setShowAddModal(true)} className="bg-primary text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all">
                + Novo Registro
              </button>
           </div>
              ...
              {employees.map(emp => (
                 <div key={emp.id} className="bg-white dark:bg-slate-900 p-6 rounded-[35px] border dark:border-slate-800 flex items-center gap-4 shadow-sm group">
                    ...
                    <div className="flex-1">
                       ...
                       <p className="text-[8px] font-black text-primary uppercase mt-1">MAT: {emp.matricula}</p>
                    </div>
                    <div className="flex flex-col gap-2">
                      <button onClick={() => { setEditingEmp(emp); setNewEmpData({ name: emp.name, matricula: emp.matricula, roleFunction: emp.roleFunction || '', workShift: emp.workShift || '', password: '' }); }} className="p-2 bg-slate-50 dark:bg-slate-800 rounded-lg text-slate-600 hover:text-primary transition-colors">✏️</button>
                      <button onClick={() => onDeleteEmployee(emp.id)} className="p-2 bg-slate-50 dark:bg-slate-800 rounded-lg text-slate-300 hover:text-red-500 transition-colors">🗑️</button>
                    </div>
                 </div>
              ))}
           </div>
           ...
           {(showAddModal || editingEmp) && (
             <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6">
                <div className="bg-white dark:bg-slate-900 rounded-[44px] w-full max-w-sm p-10 shadow-2xl space-y-4">
                   <h2 className="text-sm font-black text-primary text-center uppercase tracking-widest">{editingEmp ? 'Editar Cadastro' : 'Novo Cadastro'}</h2>
                   <input type="text" placeholder="NOME" value={newEmpData.name} onChange={e => setNewEmpData({...newEmpData, name: e.target.value.toUpperCase()})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black text-center dark:text-white" />
                   <input type="text" placeholder="MATRÍCULA" value={newEmpData.matricula} onChange={e => setNewEmpData({...newEmpData, matricula: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black text-center dark:text-white" />
                   <input type="text" placeholder="FUNÇÃO" value={newEmpData.roleFunction} onChange={e => setNewEmpData({...newEmpData, roleFunction: e.target.value.toUpperCase()})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black text-center dark:text-white" />
                   <input type="text" placeholder="JORNADA" value={newEmpData.workShift} onChange={e => setNewEmpData({...newEmpData, workShift: e.target.value.toUpperCase()})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black text-center dark:text-white" />
                   <input type="password" placeholder="SENHA INICIAL" value={newEmpData.password} onChange={e => setNewEmpData({...newEmpData, password: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-[10px] font-black text-center dark:text-white" />
                   <div className="flex gap-3 pt-4">
                      ...
                      <button onClick={() => { onAddEmployee(newEmpData); setShowAddModal(false); setEditingEmp(null); }} className="flex-[2] py-4 bg-primary text-white rounded-2xl text-[10px] font-black uppercase shadow-xl">Salvar</button>
                   </div>
                </div>
             </div>
           )}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center py-20 animate-in fade-in">
       ...
       <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Módulo em Atualização</h3>
