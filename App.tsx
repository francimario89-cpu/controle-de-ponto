...
const App: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'mypoint' | 'card' | 'requests' | 'colaboradores' | 'profile' | 'jornada' | 'calendario' | 'vacations' | 'aprovacoes' | 'contabilidade' | 'chat'>('dashboard');
  ...
  const handleLogin = (newUser: User, newCompany?: Company) => {
    ...
    setActiveView(newUser.role === 'admin' ? 'colaboradores' : 'dashboard');
  };
      ...
      <div className={`h-full w-full max-w-lg shadow-2xl flex flex-col relative border-x overflow-hidden md:rounded-[40px] transition-all duration-500 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar pb-10">
          <div className="max-w-md mx-auto w-full h-full">
            ...
            {(['colaboradores', 'jornada', 'calendario', 'vacations', 'aprovacoes', 'contabilidade'].includes(activeView)) && 
              <AdminDashboard 
